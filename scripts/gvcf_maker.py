...
def implement(args1, args2, args3, args4, args5):
    flag_ref=0
    for filename in os.listdir(args4):
        if filename.endswith(".fa"):
            ref_genome=filename
            flag_ref = 1
            break
    if flag_ref==0:
        print("ERROR: Reference genome cannot be found in RefGenome directory. Check Readme file for instructions.")
        sys.exit()
            
    for sample_file in os.listdir(args2):
        if sample_file.endswith(".csv"):
            break

    df_sample=pd.read_csv(args2+sample_file,dtype=str, sep='\t')
    ...
    csv_li = []
    for filename in os.listdir(args2):
        if filename.endswith('.csv'):
            df_csv = pd.read_csv(args2+filename, index_col=None, sep='\t')
            df_csv = df_csv.sort_values(by=['name'])
            csv_li.append(df_csv)
    df0 = pd.concat(csv_li, axis=0, ignore_index=True)

    #with open(args5+'scripts/chr4_survivor_with_svtype.list') as sv_ls:
    ...
    for filename in os.listdir(args2):
        if filename.endswith('.log'):
            ...
            ## only longranger annotation
                ...
                #print(filename, window_bgn, sv_type_lr )
                #continue
            with open(args2+filename, 'r') as info:
                row1=[]
                beginning_loc_all = []
                ending_loc_all = []
                svlen_all = []
                ...
                if failed_flag==0 and deletion_exists==1:    
                    csv_name=args2+str(filename).split('.')[0]+'.csv'
                ...
                if failed_flag==0 and deletion_exists==0:
                    continue
                if beginning_loc != 0:
                    with open(args4+'test.bed', 'w') as tb:
                        tb.write(chr_name+"\t"+str(beginning_loc)+"\t"+str(int(beginning_loc)+1)+"\ta")
                    tb.close()
                    out = subprocess.run(['bedtools getfasta -fi '+args4+ref_genome+' -bed '+args4+'test.bed'], shell=True,  capture_output=True)
    ...
    csv_li_dup = []
    for filename in os.listdir(args3):
        if filename.endswith('.csv'):
            df_csv_dup = pd.read_csv(args3+filename, index_col=None, sep=',')
    ...
    with open(args5+'sv_list_with_CI.list') as f_ci:
    ...
    for filename in os.listdir(args3):
        ...
        #        sv_type_lr=line_sv.split('\t')[2]
        if filename.endswith('.csv'):    
            df_csv_dup=pd.read_csv(args3+filename,dtype=str, sep=',')
            df_csv_dup = df_csv_dup.sort_values(by=['name'])
            ...
            with open(args4+'test.bed', 'w') as tb_dup:
                tb_dup.write(chr_name_dup+"\t"+str(beginning_loc_dup)+"\t"+str(int(beginning_loc_dup)+1)+"\ta")
            tb_dup.close()
            out_dup = subprocess.run(['bedtools getfasta -fi '+ref_genome+' -bed '+args4+'test.bed'], shell=True,  capture_output=True)
    ...
    df_chr.to_csv('./'+args1+'/'+args1+'_without_header.gvcf', header=True, index=False, sep='\t', mode='w')
